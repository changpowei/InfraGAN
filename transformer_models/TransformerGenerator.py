...
class Transformer(nn.Module):
    ...
    def forward(self, mask, content):
        ...
        hs = self.decoder(content, content, memory_key_padding_mask=mask,
                          pos=pos_embed_c, query_pos=pos_embed_c)[0]
...
class TransformerGenerator(nn.Module):
    ...
    def forward(self, input):
        ...
        samples_normalized = (samples_c.tensors - torch.min(samples_c.tensors)) / (torch.max(samples_c.tensors) - torch.min(samples_c.tensors))

        content = self.embedding(samples_normalized)
        mask = None
